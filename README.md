...
- **Verified egress IP:** `/pool?fresh=1` verifies each healthy backend by requesting `egress_verify_target` through that backend's HTTP proxy and caches the resulting `verified_proxy_ip` for `egress_verify_ttl` seconds. Country and city shown in the dashboard come from gluetun's `/v1/publicip/ip` metadata and are not cleared by egress-cache refreshes; they change when gluetun reports a new public IP/location. Diversity checks prefer fresh verified proxy IPs over gluetun's `/v1/publicip/ip` value. Stale verified proxy IP values are hidden in the dashboard, so persisted state after `down/up` does not look current. A `public_ip_mismatch` is visible degraded state, but not an automatic repair trigger by itself.
