...
- **Verified egress IP:** `/pool?fresh=1` verifies each healthy backend by requesting `egress_verify_target` through that backend's HTTP proxy and caches the resulting `verified_proxy_ip`, country, and city for `egress_verify_ttl` seconds. Diversity checks prefer fresh verified proxy IPs over gluetun's `/v1/publicip/ip` value. Stale verified proxy IP/location values are hidden in the dashboard and cleared when a backend's public IP changes, so persisted state after `down/up` does not look current. A `public_ip_mismatch` is visible degraded state, but not an automatic repair trigger by itself.
