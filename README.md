...
- **Controller state volume permissions:** the controller keeps only the startup capabilities needed to fix `/data` ownership and switch to the non-root `chamosel` user (`CHOWN`, `SETGID`, `SETUID`). Existing state volumes created by older root-running images remain writable without running the controller process as root.
