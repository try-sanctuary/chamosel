...
- **Controller state volume permissions:** the controller keeps only the `CHOWN` capability during startup so it can fix ownership of `/data` before dropping to the non-root `chamosel` user. Existing state volumes created by older root-running images remain writable without running the controller process as root.
