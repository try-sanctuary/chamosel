...
- **Controller state volume permissions:** the controller runs as a non-root user. On startup it fixes ownership of `/data` before dropping privileges, so existing state volumes created by older root-running images remain writable.
