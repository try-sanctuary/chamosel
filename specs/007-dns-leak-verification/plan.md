...
Add a read-only `verify-dns` operator command that checks each healthy backend's DNS resolver path through its HTTP proxy, reports resolver metadata in table/JSON form, and documents cautious live validation. The implementation extends the existing CLI leak-verification path and reuses `/pool?fresh=1` and backend proxy probe execution. Post-live validation also hardens controller duplicate-IP repair so rotation recovery can succeed based on fresh verified proxy IP when gluetun's public IP remains stale or mismatched.
...
**Constraints**: No provider credentials or auth tokens in output; DNS verification must not trigger automatic rotation or repair; controller auth headers must be honored when fetching pool state; no new controller dependencies.
...
- **Health-Aware Rotation**: Pass. DNS verification is read-only and never changes gluetun status, cooldowns, HAProxy health checks, or forced rotation behavior. Duplicate-IP repair recovery now uses verified proxy IP as the diversity truth when repairing duplicate egress IPs.
...
controller/controller.py       # duplicate repair recovery semantics for verified proxy IP
README.md                      # operator docs and live validation sequence
tests/test_verify_leaks.py     # CLI/report tests for DNS verification
tests/test_controller.py       # repair/recovery tests for verified proxy IP semantics
```

**Structure Decision**: Keep DNS verification in `chamosel.py` alongside existing `verify-leaks` because it reuses the same pool fetch, Docker Compose backend probe, JSON/table rendering, and CLI parser patterns. Keep duplicate repair semantics in `controller/controller.py` because the controller owns rotation, cooldown, recovery, and persisted state.
