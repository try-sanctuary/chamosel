...
2. **Given** a healthy backend and a DNS leak service result with resolver metadata that differs from the backend connection identity, **When** the operator runs DNS verification, **Then** the report marks that backend as suspicious and the command exits non-zero.
3. **Given** an unhealthy backend, **When** the operator runs DNS verification, **Then** that backend is skipped with an operator-visible reason and the overall result fails unless all configured backends are healthy and verifiable.
...
- **FR-004**: System MUST return non-zero when any healthy backend cannot be DNS-verified or is marked suspicious.
...
- Resolver ASN mismatch is treated as suspicious because it can indicate DNS traffic is using a different network path than the backend connection IP; operators may still need to interpret provider-specific DNS behavior.
